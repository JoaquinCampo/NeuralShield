...
from neuralshield.encoding.models.byt5 import ByT5Encoder
...
def main(
    ...
    model_name: str = typer.Option(
        "google/byt5-small",
        "--model",
        help="ByT5 model name",
    ),
    max_length: int = typer.Option(1024, help="Maximum sequence length"),
    ...
    """Generate byte-level embeddings from a dataset using ByT5."""
    logger.info(
        "Generating ByT5 embeddings: model={model}, device={device}",
        model=model_name,
        device=device,
    )
    ...
    # Initialize encoder
    logger.info("Loading ByT5 model: {model}", model=model_name)
    encoder = ByT5Encoder(model_name=model_name, device=device, max_length=max_length)
    logger.info("Model loaded: output_dim={dim}", dim=encoder._output_dim)
