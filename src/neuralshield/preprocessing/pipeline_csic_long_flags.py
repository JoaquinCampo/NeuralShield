"""CSIC overfit pipeline that inflates selected flag tokens."""
...
STEP_PATHS = [
    ...
    "neuralshield.preprocessing.steps.long_flag_postprocessor:LongFlagTokenAugmenter",
]


def build_csic_long_flag_pipeline(
    *, max_workers: int | None = None
) -> PreprocessorPipeline:
    """Construct the CSIC overfit pipeline with stretched flag tokens."""

    steps = [resolve(name) for name in STEP_PATHS]
    return PreprocessorPipeline(steps, max_workers=max_workers)


preprocess_csic_long_flags: PreprocessorPipeline = build_csic_long_flag_pipeline()
