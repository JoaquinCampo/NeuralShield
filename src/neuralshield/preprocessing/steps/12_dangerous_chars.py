...
class DangerousCharacterDetector(HttpPreprocessor):
    ...
    def process(self, request: str) -> str:
        ...
        for line in lines:
            ...
            elif line.startswith("[QPARAM] "):
                content = line[9:]  # Remove '[QPARAM] ' prefix
                # Extract just the key part (before any existing flags)
                content_parts = content.split()
                key_content = content_parts[0] if content_parts else content
                processed_content, flags = self._process_line_content(
                    key_content, ComponentType.QUERY
                )
                # Preserve any existing flags and add new ones
                existing_flags = content_parts[1:] if len(content_parts) > 1 else []
                all_flags = existing_flags + flags
                if all_flags:
                    processed_lines.append(f"[QPARAM] {processed_content} {' '.join(all_flags)}")
                else:
                    processed_lines.append(f"[QPARAM] {processed_content}")
