...
class QueryProcessor(HttpPreprocessor):
    ...
    def _format_query_output(
        ...
        """
        Format query processing output matching expected test format.

        Expected format:
        [QPARAM] {key} [individual_flags...]
        ...
        if result.parameter_count > 0:
            ...
            for param in result.parameters:
                ...
                # Use <empty> for empty keys for display
                ...
                key_for_count = param.key if param.key else "<empty>"
                if key_counts.get(key_for_count, 0) > 1:
            ...
            # Add global metadata with unique global flags only
            global_flags = []
            unique_flags = set()
            
            for flag in flags:
                # Skip separator flags as they're handled separately
                if flag in [QSEMISEP_FLAG, QRAWSEMI_FLAG]:
                    continue
                # Add unique global flags (including QARRAY and QREPEAT)
                if flag not in unique_flags:
                    unique_flags.add(flag)
                    global_flags.append(flag)
            
            if global_flags:
                # Sort flags to match expected order
                lines.append(
                    f"[QMETA] count={result.parameter_count} {' '.join(sorted(global_flags))}"
                )
            else:
