    ...
    def _format_query_output(
        ...
        if result.parameter_count > 0:
            ...
            for param in result.parameters:
                ...
                # Build param representation with value when applicable
                if param.is_bare:
                    param_repr = f"{display_key}"
                else:
                    param_repr = f"{display_key}={param.value}"

                # Format parameter line
                if param_flags:
                    lines.append(f"[QPARAM] {param_repr} {' '.join(param_flags)}")
                else:
                    lines.append(f"[QPARAM] {param_repr}")
