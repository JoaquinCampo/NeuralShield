"""SR_BH-specific preprocessing pipeline with aggressive heuristics enabled."""
...
STEP_PATHS = [
    ...
    "neuralshield.preprocessing.steps.05_dangerous_characters_script_mixing:DangerousCharactersScriptMixingSrbh",
    ...
    "neuralshield.preprocessing.steps.10_query_parser_and_flags:QueryParserAndFlagsSrbh",
    "neuralshield.preprocessing.steps.11_path_structure_normalizer:PathStructureNormalizerSrbh",
    "neuralshield.preprocessing.steps.12_flag_summary:FlagSummaryEmitterSrbh",
]


def build_srbh_overfit_pipeline(
    *, max_workers: int | None = None
) -> PreprocessorPipeline:
    """Construct the SR_BH-specific overfitted pipeline."""

    steps = [resolve(name) for name in STEP_PATHS]
    return PreprocessorPipeline(steps, max_workers=max_workers)


preprocess_srbh_overfit: PreprocessorPipeline = build_srbh_overfit_pipeline()
