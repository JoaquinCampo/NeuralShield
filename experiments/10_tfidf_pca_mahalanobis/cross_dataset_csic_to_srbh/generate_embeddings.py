#!/usr/bin/env python3
"""Generate TF-IDF embeddings for cross-dataset: CSIC train -> SRBH test."""
...
OUTPUT_DIR = Path("experiments/10_tfidf_pca_mahalanobis/cross_dataset_csic_to_srbh")
...
def main():
    logger.info("=" * 80)
    logger.info("Cross-Dataset TF-IDF Embedding Generation: CSIC -> SRBH")
    logger.info("=" * 80)
    ...
    # Load CSIC train data
    logger.info("\n" + "=" * 80)
    logger.info("CSIC TRAIN DATA")
    logger.info("=" * 80)
    train_requests, train_labels = load_data(CSIC_TRAIN)
    ...
    train_output_path = OUTPUT_DIR / "csic_train_embeddings.npz"
    np.savez_compressed(
        train_output_path,
    ...
    # Load SRBH test data
    logger.info("\n" + "=" * 80)
    logger.info("SRBH TEST DATA")
    logger.info("=" * 80)
    test_requests, test_labels = load_data(SRBH_TEST)
    ...
    test_output_path = OUTPUT_DIR / "srbh_test_embeddings.npz"
    np.savez_compressed(
        test_output_path,
