#!/usr/bin/env python3
"""Train IsolationForest detector on adapted SecBERT embeddings."""
...
def plot_score_distribution(
    ...
    plt.title(
        "Score Distribution: Adapted SecBERT + IsolationForest",
        fontsize=14,
        fontweight="bold",
    )
    plt.legend()
...
def plot_confusion_matrix(y_true: np.ndarray, y_pred: np.ndarray, output_path: Path):
    ...
    plt.title(
        "Confusion Matrix: Adapted SecBERT + IsolationForest",
        fontsize=14,
        fontweight="bold",
    )
...
def main(
    ...
    output_dir: Path = typer.Option(
        "experiments/11_secbert_adapted/isolation_forest",
    ...
    wandb_run_name: str = typer.Option(
        "exp11-adapted-secbert-iforest", help="W&B run name"
    ),
) -> None:
    """Train IsolationForest detector on adapted SecBERT embeddings."""
    ...
    # Load data
    logger.info("=" * 80)
    logger.info("ADAPTED SECBERT + ISOLATION FOREST")
    logger.info("=" * 80)
    ...
    logger.info("=" * 80)
    raw_scores = detector.score_samples(test_emb)
