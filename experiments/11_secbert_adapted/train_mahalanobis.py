#!/usr/bin/env python3
"""Train Mahalanobis detector on adapted SecBERT embeddings."""
...
def plot_score_distribution(
    ...
    plt.title(
        "Score Distribution: Adapted SecBERT + Mahalanobis",
        fontsize=14,
        fontweight="bold",
    )
    plt.legend()
...
def plot_confusion_matrix(y_true: np.ndarray, y_pred: np.ndarray, output_path: Path):
    ...
    plt.title("Confusion Matrix: Adapted SecBERT", fontsize=14, fontweight="bold")
...
def main(
    ...
    wandb_run_name: str = typer.Option(
        "exp11-adapted-secbert-mahalanobis", help="W&B run name"
    ),
) -> None:
    """Train Mahalanobis detector on adapted SecBERT embeddings."""
    ...
    # Load data
    logger.info("=" * 80)
    logger.info("ADAPTED SECBERT + MAHALANOBIS")
    logger.info("=" * 80)
    ...
    logger.info("=" * 80)
    test_scores = detector.mahalanobis(test_emb)
    ...
    baseline_recall = 0.4926  # From experiment 03
    improvement = recall - baseline_recall
    logger.info("=" * 80)
    logger.info("COMPARISON WITH BASELINE")
    logger.info("=" * 80)
    logger.info(f"  Baseline SecBERT (exp 03): {baseline_recall:.2%}")
    logger.info(f"  Adapted SecBERT (exp 11):  {recall:.2%}")
    logger.info(
        f"  Improvement: {improvement:+.2%} ({improvement / baseline_recall:+.1%})"
    )

    # Save results
    results = {
        "model": "adapted-secbert-mahalanobis",
        ...
        "improvement_pct": float(improvement / baseline_recall),
    }
    ...
    if use_wandb and wandb_run is not None:
        ...
        wandb.log(
            {
                ...
                "improvement_pct": improvement / baseline_recall,
                # Visualizations
        ...
        wandb.summary["baseline_recall"] = baseline_recall
