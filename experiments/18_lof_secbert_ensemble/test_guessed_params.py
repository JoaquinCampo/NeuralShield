#!/usr/bin/env python3
"""
Test the guessed hyperparameters directly (no Optuna, no grid search).
"""
...
def main() -> None:
    logger.info("Testing guessed hyperparameters:")
    logger.info("- PCA components: 150")
    logger.info("- LOF neighbors: 100")
    logger.info("- LOF contamination: auto")
    logger.info("- Fusion weight: 0.5")
    ...
    # TF-IDF vectorization (5000 dims, ngram 1-3)
    ...
    # Train LOF on TF-IDF + PCA (MY GUESSED HYPERPARAMS)
    ...
    # Score-level fusion (MY GUESSED WEIGHT = 0.5)
    ...
    # Save results
    output_dir = Path("experiments/18_lof_secbert_ensemble/guessed_params_test")
