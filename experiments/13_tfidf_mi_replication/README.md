...
## Findings (2025-10-22 update)

- **MI + OCSVM (paper baseline)**
  * Best recall: 23.3 % (`k=64`), but FPR spikes to 52 %.
  * Lowest-FPR operating point (`k=50`): 19.9 % recall @ 10.8 % FPR.
  * Selected features are mainly CSIC-specific URLs generated by ModSecurity crawlers rather than genuine payload tokens.

- **MI + Mahalanobis (fast variant)**
  * Replacing OCSVM with EmpiricalCovariance cuts runtime to seconds.
  * Best run: `k=100` → 17.3 % recall @ 4.0 % FPR. Accuracy remains far below the paper’s 70–90 % recall claims.

- **Specialist token list (Table 4.3)**
  * CSIC: tokens absent in the synthetic attacks → recall = 0 %.
  * PKDD: tokens occur (`≈5×10⁴` matches in attacks) → 18.5 % recall @ 4.6 % FPR.
  * SR-BH: tokens almost never appear (15 hits across 54 k attacks) → recall ≈ 0 %.

**Takeaway:** our public datasets (CSIC, SR-BH, PKDD) do not share the same HTTP structure as the paper’s Drupal logs. MI therefore latches onto dataset-specific artefacts and the specialist dictionary rarely fires, making high recalls unattainable without access to comparable traffic or enriched feature engineering.
