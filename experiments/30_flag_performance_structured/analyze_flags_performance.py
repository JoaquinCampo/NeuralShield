...
def process_batch(
    ...
    throughput = (
        (len(requests) / batch_time_ms * 1000) if batch_time_ms > 0 else 0.0
    )
    ...
    return {
        ...
        "avg_time_per_request_ms": (
            batch_time_ms / len(requests) if requests else 0.0
        ),
...
def main(
    ...
    # Initialize WandB
    run_name = wandb_run_name or f"flag-analysis-structured-{dataset_path.stem}"
    wandb.init(
        project=wandb_project,
        name=run_name,
        tags=["flag-analysis", "performance", "structured", "tables"],
        config={
    ...
    if batch_metrics:
        ...
        overall_p95 = np.mean(
            [b["p95_preprocessing_time_ms"] for b in batch_metrics]
        )
        overall_p99 = np.mean(
            [b["p99_preprocessing_time_ms"] for b in batch_metrics]
        )
    else:
