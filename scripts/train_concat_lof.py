#!/usr/bin/env python3
"""Train LOF detector on concatenated TF-IDF/PCA and SecBERT embeddings."""
...
app = typer.Typer(help="Train a LOF detector on concatenated embeddings.")
...
def main(
    ...
    output_dir: Path = typer.Argument(..., help="Directory to store outputs"),
    max_fpr: float = typer.Option(0.05, help="Target false positive rate"),
    n_neighbors: int = typer.Option(100, help="Number of neighbours for LOF"),
    ...
    logger.info(
        "Concatenating embeddings (train dims {} + {}, test dims {} + {})",
        tfidf_train_embeddings.shape[1],
        secbert_train_embeddings.shape[1],
        tfidf_test_embeddings.shape[1],
        secbert_test_embeddings.shape[1],
    )
    ...
    metrics = {
        "model": "LOF_concat_secbert_tfidf",
        "max_fpr": max_fpr,
        "threshold": threshold,
    ...
    (output_dir / "metrics_lof.json").write_text(json.dumps(metrics, indent=2))
    logger.info("Saved metrics to {}", output_dir / "metrics_lof.json")

    payload = {
        "name": "lof_concat_secbert_tfidf",
        "detector": detector,
        "threshold": threshold,
        "n_neighbors": n_neighbors,
        "contamination": contamination,
    ...
    joblib.dump(payload, output_dir / "concat_lof.joblib")
    logger.info("Saved concatenated LOF model to {}", output_dir / "concat_lof.joblib")
